...
- 시간 또는 토큰을 줄일 수 있는 대안을 최소 두 가지 제시하고, 각 대안의 이유와 동일한 예측 범위를 제공합니다.
...
절약안 1: Terra + high 단일 에이전트 — 통합 작업이 많아 병렬 조정 비용을 피할 수 있음
  - 7~12시간 · 0.9M~1.5M 토큰 · 비용은 현재 단가 확인 후 산정
절약안 2: 대표 소스만 먼저 샘플링 — 전체 수집 전 품질 기준을 빠르게 검증
  - 2~4시간 · 0.3M~0.6M 토큰 · 비용은 현재 단가 확인 후 산정
```
