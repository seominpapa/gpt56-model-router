...
- 30분 이상으로 예상되는 프로젝트는 기준 추천안을 보존합니다. 사용자가 진행 중 조정을 요청하면 근거를 평가하고, 다음 작업 단위 또는 새 하위 에이전트에 적용할 조정안을 제시합니다.
...
초기에는 기준 추천안과 절약 대안만 제시하며, 설정 변경 여부를 묻지 않습니다. 예상 소요 시간이 **30분 이상**인 프로젝트에서 사용자가 “모델 또는 추론강도를 조정해줘”라고 요청하면 아래 항목을 기준안과 비교합니다.
...
사용자가 조정을 요청한 뒤에만 지원되는 Codex 환경에서 **다음 작업 단위 또는 새 하위 에이전트**에 적용합니다. 실행 중인 루트 에이전트의 모델을 바꾸지는 않습니다. 백그라운드 모니터링도 별도의 사용자 승인 없이는 만들지 않으며, 모니터는 자동 변경 대신 조정안을 보고해야 합니다.
