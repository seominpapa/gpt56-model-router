...
Before changing settings, report the old and proposed model/reasoning effort, evidence and reason, predicted quality/time/token/cost impact, and whether it applies to the next work unit or a new subagent. Apply `fixed` as report-only; ask exactly once for `confirm once`, then honor its approval or rejection for the rest of the project; apply justified settings automatically for `automatic`. Adjust one dimension at a time: effort before model tier; use parallel subagents only for independent bottlenecks; wait for validation before another adjustment. Never change the running root agent. Read `references/adjustment-workflow.md` for the complete workflow.
